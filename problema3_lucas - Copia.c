...
typedef struct_dados{	// Estrutura de Dados de Entrada
	// a hora nao é armazenada, já que tem um indice para isso
    float temp;
    float pressao;
    float umidade;
    float prec;
	...
	while(true)
	{
		...
		switch (escolha)
		{
			...
			case 2:
				...
				break;
			case 3:
				printf("Informe a sigla, o dia e a hora que deseja consultar, separado por um espaço\n");
				...
				break;
			case 4:
				printf("Digite o dia em que deseja gerar as médias");
				scanf("%d",&dia_media);
...
void consulta(char sigla[3], int dia, int hora)
{
	...
	printf("Temperatura: |%f|,  Pressão Atmosferica: |%f|,  Umidade: |%f|,  Precipitação: |%f|, Km: |%f|,  Vento:  |%s|",matriz[cidade][dia][hora].temp,matriz[cidade][dia][hora].pressao,matriz[cidade][dia][hora].umidade,matriz[cidade][dia][hora].prec,matriz[cidade][dia][hora].km,matriz[cidade][dia][hora].vento);
...
void selection_sort(saida vet[])
{
    ...
    for (i = 0; i < 10; i++)
    {
		...
		a[i].temp = a[min].temp;
		a[min].temp = aux;
		//o campo CIDADE também acompanha a troca para nao se perder
		aux = a[i].cidade;
